...
import {
  useEvents, VENDOR_POOL, fmtDate,
  VendorService, AssignedVendor,
...
export default function VendorManagement() {
  ...
  const { events, assignVendor, removeVendor, updateEventVendorServices } = useEvents()

  const event = events.find(e => e.id === id)

  // Add-vendor-from-pool state
  ...
  const handleAddNew = (serviceId: number) => {
    ...
    setNewCompany(''); setNewContact(''); setNewPhone('')
    setNewSvcType(''); setNewQuote('')
    setAddingServiceId(null)
  }
  ...
  const handleAddService = () => {
    ...
    const svc: VendorService = {
      ...
      budget:      Number(newServiceBudget) || 0,
      count:       1,
      type:        newServiceType,
    }
    updateEventVendorServices(event.id, [...services, svc])
    setNewServiceName(''); setNewServiceDesc(''); setNewServiceBudget('')
  ...
  const totalVendorCost   = services
    .filter(s => (s.type || 'service') === 'service')
    .reduce((a, s) => a + (assignments[s.id.toString()] || []).reduce((b, vn) => b + vn.quote, 0), 0)
  const totalBoothRevenue = services
    ...
    <div className="p-4 md:p-8 max-w-4xl">
      ...
      <div className="space-y-4">
        ...
        {services.map(svc => {
          ...
          const poolFiltered = VENDOR_POOL.filter(vp =>
                    ...
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      {isParticipant ? (
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700">
                          Booth Rental · {assigned.length}/{svc.count} slots
                        </span>
                      ) : (
                ...
                {isParticipant ? (
                  <div className="flex items-center gap-4 text-xs mb-3">
                    <span className="text-slate-500">
                      Booth Fee: <span className="font-semibold text-slate-900">${svc.budget.toLocaleString()} per slot</span>
                    </span>
                    {quotedAmt > 0 && (
                      <span className="flex items-center gap-1 font-semibold text-emerald-600">
                        <CheckCircle2 size={12} />
                        Revenue: ${quotedAmt.toLocaleString()} from {assigned.length} vendor{assigned.length !== 1 ? 's' : ''}
                      </span>
                    )}
                  </div>
                ) : (
                  <div className="flex items-center gap-4 text-xs mb-3">
                    <span className="text-slate-500">
                      Budget: <span className="font-semibold text-slate-900">${svc.budget.toLocaleString()}</span>
                    </span>
                    {quotedAmt > 0 && (
                      <span className={`flex items-center gap-1 font-semibold ${overBudget ? 'text-red-600' : 'text-emerald-600'}`}>
                        ...
                        {overBudget
                          ? `$${(quotedAmt - svc.budget).toLocaleString()} over budget`
                          : `$${(svc.budget - quotedAmt).toLocaleString()} under budget`}
                      </span>
                    )}
                  </div>
                )}
                    ...
                    {assigned.map(vendor => (
                      <div key={vendor.id} className="flex items-center justify-between gap-2 p-2.5 bg-slate-50 rounded-lg">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <button
                            onClick={() => removeVendor(event.id, svc.id, vendor.id)}
              ...
              {isAdding && (
                <div className="border-t border-slate-100 bg-slate-50 p-4 md:p-5">
                  ...
                  <div className="relative mb-3">
                    ...
                    <input
                      type="text"
                      placeholder="Search vendor pool..."
                      value={search}
                  ...
                  <div className="space-y-1.5 mb-3 max-h-48 overflow-y-auto">
                    {poolFiltered.length === 0 && (
                      <p className="text-slate-400 text-xs py-2 text-center">No matching vendors in pool</p>
                    )}
                    {poolFiltered.map(vendor => (
                      <button
                        key={vendor.id}
                        onClick={() => { setSelectedVendor(vendor); setQuoteInput('') }}
                        className={`w-full flex items-center justify-between p-2.5 rounded-lg text-left transition-colors ${
                          selectedVendor?.id === vendor.id
                            ? 'bg-amber-50 border border-amber-200'
                            : 'bg-white border border-slate-200 hover:border-amber-200 hover:bg-amber-50/40'
                        }`}
                      >
                        <div className="flex items-center gap-2.5">
                          <Building2 size={14} className="text-slate-400" />
                          <div>
                            <p className="text-sm font-semibold text-slate-900">{vendor.company}</p>
                            <p className="text-xs text-slate-500">{vendor.service} · {vendor.contact}</p>
                          </div>
                        </div>
                  ...
                  {/* Quote input when vendor selected */}
                  {selectedVendor && (
                    <div className="bg-white border border-amber-200 rounded-lg p-3 mb-3">
                      <p className="text-xs font-semibold text-slate-700 mb-2">
                        Adding <span className="text-amber-700">{selectedVendor.company}</span>
                      </p>
                      <div className="flex gap-2">
                        <div className="relative flex-1">
                          ...
                          <input
                            type="number"
                            placeholder={isParticipant ? 'Booth fee they pay' : 'Quote amount'}
                            value={quoteInput}
                        ...
                        <button
                          onClick={() => handleConfirmAdd(svc.id)}
                          disabled={!quoteInput}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <input
                        ...
                        className="px-3 py-2 border border-slate-200 bg-white rounded-lg text-sm focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-50"
                      />
                      <input
                        type="text"
                      ...
                      <select
                        value={newTier}
                        onChange={e => setNewTier(e.target.value as AssignedVendor['tier'])}
                        className="px-3 py-2 border border-slate-200 bg-white rounded-lg text-sm focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-50"
                      >
                    ...
                    <div className="flex gap-2">
                      <div className="relative flex-1">
                        ...
                        <input
                          ...
                          className="pl-6 pr-3 py-2 w-full border border-slate-200 bg-white rounded-lg text-sm focus:outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-50"
                        />
                      </div>
                      <button
                        onClick={() => handleAddNew(svc.id)}
                        disabled={!newCompany.trim()}
                        className="px-4 py-2 bg-slate-900 text-white rounded-lg text-sm font-semibold hover:bg-slate-800 disabled:opacity-40 transition-colors"
                      >
                        Add New
                      </button>
                    </div>
        ...
        <div className="bg-white rounded-xl border border-dashed border-slate-300 overflow-hidden">
          <button
            ...
            className="w-full flex items-center gap-2 px-5 py-4 text-sm font-semibold text-amber-600 hover:text-amber-700 hover:bg-amber-50/30 transition-colors"
          >
            <Plus size={16} className={`transition-transform ${addingService ? 'rotate-45' : ''}`} />
            Add Service Requirement
          </button>
          ...
          {addingService && (
            <div className="border-t border-slate-100 bg-slate-50 p-4 space-y-2">
              ...
              <div className="flex gap-1 p-1 bg-slate-100 rounded-lg mb-1">
                <button
                  onClick={() => setNewServiceType('service')}
                  className={`flex-1 py-1.5 px-2 rounded-md text-xs font-semibold transition-colors ${newServiceType === 'service' ? 'bg-white text-amber-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  Service Provider (you pay them)
                </button>
                <button
                  onClick={() => setNewServiceType('participant')}
                  className={`flex-1 py-1.5 px-2 rounded-md text-xs font-semibold transition-colors ${newServiceType === 'participant' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  Booth / Exhibitor (they pay you)
                </button>
              </div>
              <input
                type="text"
                placeholder={newServiceType === 'participant' ? 'Booth type (e.g. F&B Stalls, Retail Booths) *' : 'Service name (e.g. Catering, AV & Sound) *'}
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  ...
                  <input
                    type="number"
                    placeholder={newServiceType === 'participant' ? 'Booth fee per vendor' : 'Budget'}
