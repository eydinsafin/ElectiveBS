...
import {
  ArrowLeft, Edit2, DollarSign, MapPin, Clock, Calendar, Users, Check, X, Plus, Minus,
} from 'lucide-react'
...
export default function EventDetails() {
  ...
  const { events, updateEvent, updateEventRoles } = useEvents()
  ...
  if (!event) {
    return (
      <div className="p-8 text-center">
        ...
        <button onClick={() => navigate('/events')} className="text-blue-600 font-semibold text-sm">
          ← Back to Events
        </button>
      </div>
    )
  ...
  // Flatten assigned staff from all roles
        ...
        <div className="min-w-0">
          {editing ? (
            ...
            <div className="flex items-center gap-3 mb-1.5 flex-wrap">
              ...
              <span className={`px-2.5 py-1 text-xs font-bold rounded-full ${STATUS_BADGE[event.status]}`}>
                {event.status}
              </span>
            </div>
          )}
          <div className="flex flex-wrap gap-4 text-slate-400 text-sm">
            <span className="flex items-center gap-1.5">
              <Clock size={14} />
              {fmtDate(event.date)} · {fmtTime(event.from)}{event.to ? ` – ${fmtTime(event.to)}` : ''}
            </span>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {editing ? (
            <>
              <button
                onClick={cancelEdit}
                className="flex items-center gap-2 px-3 py-2 border border-slate-200 bg-white text-slate-700 rounded-lg text-sm font-semibold hover:bg-slate-50 transition-colors"
              >
                <X size={14} /> Cancel
              </button>
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 bg-emerald-600 text-white rounded-lg text-sm font-semibold hover:bg-emerald-700 disabled:opacity-40 transition-colors shadow-md shadow-emerald-600/20"
              >
            ...
            <>
              ...
              <button
                onClick={() => navigate('/payroll/PAY-2023-10-B')}
                className="flex items-center gap-2 px-3 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 transition-colors shadow-md shadow-blue-600/20"
              >
              ...
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  {['Staff Member', 'Assigned Role', 'Tier', ''].map((col, i) => (
                    <th key={i} className={`px-4 md:px-5 py-3 text-[11px] font-semibold text-slate-400 uppercase tracking-wider ${i === 4 ? 'text-right' : 'text-left'}`}>
                      {col}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-slate-100">
                {roster.length === 0 ? (
                  ...
                  <tr key={i} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 md:px-5 py-3.5">
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${TIER_BADGE[m.tier as Tier]}`}>
                        {m.tier}
                      </span>
                    </td>
        ...
        <div className="space-y-4">
              ...
              {editing ? (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="text-[11px] text-slate-400 font-semibold uppercase tracking-wider mb-1 block">Start Time</label>
                      <input
                        type="time"
                        value={draft.from}
                    ...
                    <div>
                      <label className="text-[11px] text-slate-400 font-semibold uppercase tracking-wider mb-1 block">End Time</label>
                      <input
                        type="time"
                        value={draft.to}
                  ...
                  <div>
                    ...
                    <select
                      value={draft.status}
                      onChange={e => setDraft(d => ({ ...d, status: e.target.value as typeof STATUS_OPTIONS[number] }))}
                      className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm text-slate-900 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-colors"
                    >
                  ...
                  <div className="flex gap-2 pt-1">
                    <button
                      onClick={cancelEdit}
                      className="flex-1 py-2 border border-slate-200 text-slate-700 rounded-lg text-sm font-semibold hover:bg-slate-50 transition-colors"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={saveEdit}
                      disabled={!draft.name.trim()}
                      className="flex-1 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 disabled:opacity-40 transition-colors"
                    >
                      Save
                    </button>
                  </div>
                ...
                <>
                  <div>
                    ...
                    <p className="text-sm font-semibold text-slate-900 flex items-center gap-1.5">
                      <Calendar size={13} className="text-slate-400" />{fmtDate(event.date)}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-sm font-semibold text-slate-900 flex items-center gap-1.5">
                      ...
                      {fmtTime(event.from)}{event.to ? ` – ${fmtTime(event.to)}` : ''}
                    </p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${STATUS_BADGE[event.status]}`}>
                      {event.status}
                    </span>
                  </div>
                  <div>
                    ...
                    {event.staffRoles.length === 0 && !addingRole && (
                        ...
                        return (
                          <div key={sr.id} className="flex items-center gap-2 py-1.5 border-b border-slate-50 last:border-0">
                            ...
                            <div className="flex items-center gap-1 shrink-0">
                              <button
                                onClick={() => changeRoleCount(sr.id, -1)}
                                disabled={sr.count <= filledCount || sr.count <= 1}
                                className="w-6 h-6 rounded border border-slate-200 flex items-center justify-center hover:bg-slate-100 disabled:opacity-30 transition-colors"
                              >
                                <Minus size={10} />
                              </button>
                              <span className={`text-sm font-bold w-5 text-center ${filledCount >= sr.count ? 'text-emerald-600' : 'text-amber-600'}`}>
                                {sr.count}
                              </span>
                              <button
                                onClick={() => changeRoleCount(sr.id, 1)}
                            ...
                            <button
                              onClick={() => deleteRole(sr.id)}
                              disabled={filledCount > 0}
                              title={filledCount > 0 ? 'Remove assigned staff first' : 'Delete role'}
                              className="w-6 h-6 rounded hover:bg-red-50 flex items-center justify-center shrink-0 disabled:opacity-25 transition-colors"
                            >
                    ...
                    {addingRole && (
                      <div className="mt-2 p-3 border border-slate-200 rounded-xl bg-slate-50 space-y-2">
                        ...
                        <div className="grid grid-cols-3 gap-1.5">
                          {[
                            { label: 'Count', key: 'count' as const, min: 1 },
                            { label: '$/hr',  key: 'rate'  as const, min: 0 },
                            { label: 'Hours', key: 'hours' as const, min: 1 },
                          ].map(({ label, key, min }) => (
                            <div key={key}>
                              ...
                              <input
                                ...
                                min={min}
                                value={newRole[key]}
                                onChange={e => setNewRole(r => ({ ...r, [key]: Math.max(min, parseInt(e.target.value) || min) }))}
                        ...
                        <div className="flex gap-2 pt-1">
                          <button
                            onClick={() => { setAddingRole(false); setNewRole({ role: '', responsibilities: '', count: 1, rate: 15, hours: 8 }) }}
                            className="flex-1 py-1.5 border border-slate-200 bg-white text-slate-600 rounded-lg text-xs font-semibold hover:bg-slate-100 transition-colors"
                          >
                            Cancel
                          </button>
                          <button
                            ...
                            className="flex-1 py-1.5 bg-blue-600 text-white rounded-lg text-xs font-semibold hover:bg-blue-700 disabled:opacity-40 transition-colors"
                          >
                            Add Role
                          </button>
                        </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200">
            ...
            <div className="p-5">
              ...
              <button
                onClick={() => navigate(`/events/${event.id}/staff`)}
                className="mt-4 w-full py-2.5 border border-slate-200 rounded-lg text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
              >
                Manage Staff →
              </button>
            </div>
