...
import { ArrowLeft, Users, Search, Plus, X, UserPlus, MapPin, Clock, Calendar } from 'lucide-react'
import Avatar from '../components/Avatar'
import {
  useEvents, STAFF_POOL, AssignedPerson,
...
export default function StaffManagement() {
  ...
  const { events, assignToRole, removeFromRole } = useEvents()
  ...
  const getRoleAssigned = (roleId: number): AssignedPerson[] =>
    assignments[roleId.toString()] || []
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 md:p-6 mb-5">
        ...
        <div>
          ...
          <div className="h-2 bg-slate-100 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-500 ${PROGRESS_BAR[event.status]}`}
              style={{ width: `${pct}%` }}
            />
          </div>
      ...
      <div className="space-y-4">
        {event.staffRoles.length === 0 && (
          <div className="bg-white rounded-xl border border-slate-200 p-10 text-center">
            ...
            <button onClick={() => navigate(`/events/${event.id}`)} className="text-blue-600 text-sm font-semibold">
              Edit Event →
            </button>
          </div>
        )}
              ...
              <div className="px-5 py-4 border-b border-slate-100">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    {sr.responsibilities && (
                      <p className="text-xs text-slate-400 mt-0.5">{sr.responsibilities}</p>
                    )}
                    <div className="flex flex-wrap gap-3 mt-2">
                      ...
                      <span className="text-xs text-slate-500">
                        Budget: ${(sr.rate * sr.hours * sr.count).toLocaleString()}
                      </span>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">
                      {needed > 0 ? `${needed} more needed` : 'Filled ✓'}
                    </p>
                    <div className="w-20 h-1.5 bg-slate-100 rounded-full overflow-hidden mt-1.5">
                      <div
                        className={`h-full rounded-full ${rolePct >= 100 ? 'bg-emerald-500' : rolePct >= 50 ? 'bg-amber-500' : 'bg-red-400'}`}
                        style={{ width: `${rolePct}%` }}
                      />
                    </div>
                  </div>
              ...
              <div className="px-5 py-3 pb-4">
                ...
                {assigned.length > 0 && (
                  <div className="space-y-1.5 mb-3">
                    {assigned.map(p => (
                      <div key={p.id} className="flex items-center gap-3 py-1.5 px-3 rounded-lg bg-slate-50">
                        <Avatar name={p.name} size="sm" />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-slate-900 truncate">{p.name}</p>
                          {p.phone && <p className="text-xs text-slate-400">{p.phone}</p>}
                        </div>
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded shrink-0 ${TIER_BADGE[p.tier]}`}>
                          {p.tier}
                        </span>
                        <button
                          onClick={() => removeFromRole(event.id, sr.id, p.id)}
                  ...
                  <div className="border border-slate-200 rounded-xl p-4 mb-3 bg-slate-50">
                    ...
                    <div className="relative mb-3">
                      ...
                      <input
                        ...
                        className="pl-9 pr-3 py-2 w-full border border-slate-200 bg-white rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-colors"
                        autoFocus
                      />
                    </div>
                    ...
                    {pool.length > 0 ? (
                      <div className="mb-4">
                        ...
                        <div className="space-y-1 max-h-48 overflow-y-auto">
                          {pool.map(p => (
                            <div key={p.id} className="flex items-center gap-3 py-1.5 px-2 rounded-lg hover:bg-white transition-colors">
                              <Avatar name={p.name} size="sm" />
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-slate-900 truncate">{p.name}</p>
                              ...
                              <button
                                onClick={() => assignToRole(event.id, sr.id, p)}
                                className="px-2.5 py-1 bg-blue-600 text-white rounded-lg text-xs font-semibold hover:bg-blue-700 shrink-0 transition-colors"
                              >
                                Add
                              </button>
                            </div>
                      ...
                      search && (
                        <p className="text-xs text-slate-400 text-center py-2 mb-3">No matching staff found</p>
                      )
                    )}
                    ...
                    <div className="space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        <input
                          ...
                          className="px-3 py-2 border border-slate-200 bg-white rounded-lg text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-colors"
                        />
                        <input
                          type="tel"
                      ...
                      <div className="flex items-center gap-2">
                        <select
                          ...
                          className="flex-1 px-3 py-2 border border-slate-200 bg-white rounded-lg text-sm text-slate-900 focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-50 transition-colors"
                        >
